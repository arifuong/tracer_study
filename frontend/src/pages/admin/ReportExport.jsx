...
const ReportExport = () => {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      setSuccess(`Laporan dalam format ${format.toUpperCase()} berhasil dibuat dan diunduh.`);
    } catch (err) {
      setError('Gagal mengekspor laporan. Coba kembali beberapa saat.');
    } finally {
      setExporting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="h-96 flex flex-col items-center justify-center text-slate-500 gap-3">
        ...
        <p className="text-sm font-medium">Memuat konfigurasi ekspor laporan...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Cetak & Ekspor Laporan</h1>
        <p className="text-slate-500 text-sm mt-1">Generate dokumen rekapitulasi data tracer study alumni ke PDF atau Excel.</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 shadow-sm">
        <h3 className="font-bold text-slate-800 text-base mb-6 border-b border-slate-100 pb-2">Buat Laporan Baru</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Report Name Input */}
          <div>
            <label className="block text-slate-700 text-sm font-medium mb-2">Nama File Laporan</label>
            <input
              type="text"
              value={reportName}
              onChange={(e) => setReportName(e.target.value)}
              required
              placeholder="Contoh: Laporan Tracer Study Informatika 2026"
              className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 text-sm transition-colors"
            />
          </div>
        </div>
