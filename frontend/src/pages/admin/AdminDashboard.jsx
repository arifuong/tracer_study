...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      setLoading(true);
      try {
        const url = selectedPeriod ? `/api/admin/dashboard?periodeId=${selectedPeriod}` : '/api/admin/dashboard';
        const response = await api.get(url);
        setStats(response.data);
      } catch (err) {
        setError('Gagal memuat statistik dashboard. Silakan muat ulang halaman.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Analitik Tracer Study</h1>
          <p className="text-slate-500 text-sm mt-1">Hasil penyerapan kuesioner alumni .</p>
        </div>
      ...
      {stats && academicQuestions.length > 0 && (
        <div className="space-y-6 pt-4 border-t border-slate-200/60">
          <div className="flex items-center justify-between">
            ...
            >
              <span>{showAcademic ? 'Sembunyikan Analitik Akademik' : 'Lihat Analitik'}</span>
